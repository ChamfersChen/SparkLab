...
帮助 {{角色}} 在 {{场景}} 中产出可发布的笔记"
          ...
          <section class="card-block">
            ...
            <p class="section-hint">
              ...
              <code>{`{{prev_output}}`}</code>。用户运行本步时,前一步 AI 平台返回的结果会自动注入 <code>{`{{prev_output}}`}</code>。
            </p>
                  ...
                  <a-form layout="vertical">
                    ...
                    <a-form-item label="步骤内容（Markdown）" required>
                      <div class="step-mode-bar">
                        <span class="step-mode-hint">支持 <code>{`{{变量}}`}</code> 与 <code>{`{{prev_output}}`}</code>（前者用户填,后者运行时由前一步 AI 结果自动注入）</span>
                        <div class="mode-switch">
                          <button
                            type="button"
                            class="mode-btn"

 ...
 *
 * 内容字段: 单 content 字段 (Markdown + {{var}}),与 TemplateEditor 一致。
 */
...
const stepViewModes = ref({})  // { [stepIndex]: 'edit' | 'preview' }
...
function extractVariables(text) {
  ...
  while ((m = _VAR_REGEX.exec(text || '')) !== null) {
    const k = m[1].trim()
    if (k === 'prev_output') continue  // 特殊占位符,不可填
    if (!seen.has(k)) {
      seen.add(k)
      out.push(k)
    }
...
async function fetchPlaybook() {
  ...
  loading.value = true
  try {
    const p = await adminGetPlaybook(route.params.id)
...
async function handleSave(publish = false) {
  ...
  const payload = {
    ...
    status: publish ? 'published' : form.value.status === 'published' ? 'published' : 'draft',
  }

  saving.value = true
  try {
    if (isEdit.value) {
      await adminUpdatePlaybook(route.params.id, payload)
      message.success(publish ? '已保存并发布' : '已保存')
    } else {
      const created = await adminCreatePlaybook(payload)
      message.success(publish ? '已创建并发布' : '已创建草稿')
      router.replace({ name: 'admin-playbook-edit', params: { id: created.id } })
    }
    await fetchPlaybook()
  } catch (e) {
...
function goBack() {
  router.push({ name: 'admin-playbooks' })
}
...
帮助 {{角色}} 在 {{场景}} 中产出可发布的笔记"
          ...
          <section class="card-block">
            ...
            <p class="section-hint">
              ...
              <code v-pre>{{prev_output}}</code>。用户运行本步时,前一步 AI 平台返回的结果会自动注入 <code v-pre>{{prev_output}}`}</code>。
            </p>
                  ...
                  <a-form layout="vertical">
                    <a-form-item label="步骤名" required>
                      <a-input
                        ...
                        placeholder="如「分析产品定位」「提炼卖点」「生成完整笔记」"
                        maxlength="100"
                      />
                    </a-form-item>
                    <a-form-item label="步骤内容（Markdown）" required>
                      <div class="step-mode-bar">
                        <span class="step-mode-hint">支持 <code v-pre>{{变量}}</code> 与 <code v-pre>{{prev_output}}`}</code>（前者用户填,后者运行时由前一步 AI 结果自动注入）</span>
                        <div class="mode-switch">
                          <button
                            type="button"
                            class="mode-btn"
...
请基于以下信息分析产品的目标人群与核心卖点：
- 产品：{{产品名称}}
- 行业：{{行业}}
- 上一步提炼的卖点：{{prev_output}}

输出 JSON 格式：{ persona, pain_points, value_props }"
                      />
                      <div
          ...
          <div class="submit-bar">
            ...
            <a-button :loading="saving" @click="handleSave(false)">
              <template #icon><Save :size="14" /></template>
              保存草稿
            </a-button>
            <a-button type="primary" :loading="saving" @click="handleSave(true)">
              保存并发布
            </a-button>
          </div>
        </div>

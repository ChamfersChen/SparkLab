<script setup>
/**
 * 我的模板编辑器（新建/编辑）—— 普通用户创建私有模板。
 *
 * 基于 admin/TemplateEditor.vue 简化：
 * - 去掉管理员权限检查
 * - 使用 my* API 系列
 * - 导航回 my-templates 列表
 */
...
import { ChevronLeft, Eye, Edit3, Save } from 'lucide-vue-next'
import MarkdownIt from 'markdown-it'
import { myCreateTemplate, myUpdateTemplate, myGetTemplate } from '@/apis/template_api'
...
async function fetchTemplate() {
  ...
  loading.value = true
  try {
    const t = await myGetTemplate(route.params.id)
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await myUpdateTemplate(route.params.id, payload)
      message.success('模板已更新')
      router.push({ name: 'my-templates' })
    } else {
      const created = await myCreateTemplate(payload)
      message.success('模板已创建')
      router.push({ name: 'my-template-detail', params: { id: created.id } })
    }
  } catch (e) {
      ...
      <header class="page-bar page-bar--editor">
        <div class="page-bar__left-group">
          <button
            type="button"
            class="icon-text-btn"
            @click="router.push({ name: 'my-templates' })"
          >
            <ChevronLeft :size="16" />
            <span>返回</span>
          </button>
